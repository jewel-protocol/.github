...
describe("Raffle", () => {
    describe("Contract", () => {
        const BigNum = hre.ethers.BigNumber.from(10).pow(18);
        let Raffle;
        let Link;
        let Owner;
        let NonOwner;
        ...
        it("Contract should be initialized without any ETH.", async () => { 
            const balance = await hre.ethers.provider.getBalance(Raffle.address);
            assert(balance.eq(0), `${balance} != ${0}`);
        });
        ...
        xit("Should payout when target is reached.", async () => {
            //TODO: testing this on hardhat
